...
| `REPLIT_API_URL` | Yes | Main Replit app URL (e.g., `https://your-app.replit.app`) |
| `EXTERNAL_API_KEY` | Yes | Shared API key for server authentication |
| `EXTERNAL_API_SECRET` | Yes | Shared API secret for signature generation |
...
### v1.6.2 (Current)
- Fixed: Storage method now returns created pending doc ID for observability
- Fixed: Create path includes message_sid for idempotency on retries
- Fixed: Update path now sets phone_number in addition to other metadata
- Fixed: TypeScript null safety in create-doc stage

### v1.6.1
- Fixed: Relaxed schema to allow nullable patientId/patientName
- Fixed: Added fallback for creating new pending docs when no QR scan placeholder exists
- Fixed: Added message_sid, agency_id, patient_id, patient_name, service_date to update SQL

### v1.6.0
- **Major Change**: External worker now calls server HTTP endpoint instead of direct DB writes
- Enables SSE broadcasts for real-time UI updates when docs are processed
- Requires new env vars: REPLIT_API_URL, EXTERNAL_API_KEY, EXTERNAL_API_SECRET
- API client with exponential backoff retry logic and HMAC signature generation

### v1.5.x
- Original direct SQL update implementation
- Worker wrote to DB directly, SSE events not triggered
